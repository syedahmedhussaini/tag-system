...
* `./node_modules/.bin/tailwind build tailwind-style.css -c ./tailwind.js -o ./styles/output.css` : builds the Tailwind output file (which should be copied to `content/scss/` and renamed to an scss extension) from the Tailwind config file (`tailwind.js`) and the style file (`styles.css`)
